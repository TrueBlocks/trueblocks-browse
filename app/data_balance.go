...
var balancesLock atomic.Uint32

func (a *App) loadBalances(wg *sync.WaitGroup, errorChan chan error) error {
	defer a.trackPerformance("loadBalances", false)()
	defer func() {
		if wg != nil {
			wg.Done()
		}
	...
	if !balancesLock.CompareAndSwap(0, 1) {
		return nil
	}
	defer balancesLock.CompareAndSwap(1, 0)

	// EXISTING_CODE
	// EXISTING_CODE

	if !a.isConfigured() || !a.balances.NeedsUpdate() {
		return nil
	}
	updater := a.balances.Updater
	defer func() {
		a.balances.Updater = updater
	}()
	logger.InfoBY("Updating balances...")

	if items, meta, err := a.pullBalances(); err != nil {
		if errorChan != nil {
			errorChan <- err
		}
		return err
	} else if (items == nil) || (len(items) == 0) {
		err = fmt.Errorf("no balances found")
		if errorChan != nil {
			errorChan <- err
		}
		return err
		...
		a.emitLoadingMsg(messages.Loaded, "balances")
	}

	return nil
}

func (a *App) pullBalances() (items []types.Transaction, meta *types.Meta, err error) {
	// EXISTING_CODE
	// EXISTING_CODE
	return
}
